...
export default {
    ...
    deleteSeasonData (id, isHot) {
        // let d1 = 'DELETE FROM project_params WHERE project_id = ' + id + "' AND hot=" + isHot;
        // let res1 = db.executeQuery(d1, []);
        // let d2 = 'DELETE FROM projects_params WHERE project_id = ' + id + "' AND hot=" + isHot;
        // let res2 = db.executeQuery(d1, []);
        // return db.executeQuery(q, []);
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //     let id;
            // }
            let deleteFunc = (dbClient, id, isHot) => {
                console.log('deleteFunc Id', id);
                console.log('deleteFunc isHot', isHot);
                let aveD = 'DELETE FROM projects_params WHERE project_id = ' + id + ' AND hot=' + isHot;
                let paramsD = 'DELETE FROM project_params WHERE project_id = ' + id + ' AND hot=' + isHot;
                console.log('deleteFunc', aveD);
                Promise.all([dbClient.query(aveD, []), dbClient.query(paramsD, [])])
            ...
            try {
                console.log('deleteSeasonData id', id);
                console.log('deleteSeasonData isHot', isHot);
                await db.transactions(deleteFunc, [id, isHot]);
                resolve(id)
            } catch (e) {
